...
fn load_encoder_cl100k() -> HashMap<Vec<u8>, usize> {
    let bytes = include_bytes!("cl100k_base.tiktoken");
    let mut result = HashMap::default();
    let input_str = str::from_utf8(bytes).unwrap();
    let lines = input_str.lines();
    for line in lines {
        let parts: Vec<&str> = line.split(' ').collect();
        if parts.len() == 2 {
            let key = base64::decode(parts[0]).unwrap_or_else(|_| Vec::new());
...
fn get_special_tokens_cl100k() -> HashMap<String, usize> {
    let mut my_map: HashMap<String, usize> = HashMap::default();
    // add key-value pairs
    my_map.insert("<|endoftext|>".to_string(), 100257);
    my_map.insert("<|fim_prefix|>".to_string(), 100258);
    my_map.insert("<|fim_middle|>".to_string(), 100259);
    my_map.insert("<|fim_suffix|>".to_string(), 100260);
    my_map.insert("<|endofprompt|>".to_string(), 100276);
    my_map
}

#[no_mangle]
pub extern "C" fn create_tokenizer() -> *mut CoreBPE {
    let pattern = r"(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+";
    let tret = CoreBPE::new(load_encoder_cl100k(), get_special_tokens_cl100k(), pattern);
    match tret {
    Ok(v) => {
        let _ret: *mut CoreBPE = unsafe {transmute(Box::new(v))};
        _ret
    }
    Err(e) => {
        println!("error: {:?}", e);
...
mod tests {
    ...
    fn it_works() {
        ...
        let story = "Once upon a time...Hey there!";
